    ...
    return (
        <div id="pdf" className="h-[11in] w-[8.5in] bg-white flex flex-col gap-4 p-10 text-xs">
            ...
            <section className="warranty-section">
                <p className="label col-span-3">Fecha:</p> <p className="value col-span-3">{warranty?.fecha}</p>
                <p className="label col-span-3">Número Telefónico:</p> <p className="value col-span-3">{warranty?.telefono}</p>
                <p className="label col-span-4">Nombre del cliente:</p> <p className="value name col-span-8">{warranty?.nombre}</p>
                <p className="label col-span-3">Factura:</p> <p className="value col-span-3">{warranty?.factura}</p>
                <p className="label col-span-3">Correo:</p> <p className="value col-span-3">{warranty?.correo}</p>
            </section>
            <section className="warranty-section">
                <p className="label col-span-4">Descripcion del Equipo:</p> <p className="value col-span-8">{warranty?.producto.descripcion}</p>
                <p className="label col-span-3">SKU:</p> <p className="value col-span-3">{warranty?.producto.sku}</p>
                <p className="label col-span-3">Equipo enciende:</p> <p className="value col-span-3">{warranty?.producto.enciende ? "Sí" : "No"}</p>
